  ...
  app.configure('production', function() {
    ...
    if (conf.https && conf.redir) {
      http.createSever(function(req, res) {
        return self._redirectHost('https', req, res);
      }).listen(80);
    }
  });
  ...
  app.configure('production', function() {
    if (conf.redir) {
      app.use(function(req, res, next) {
        ...
        return self._redirectHost(protocol, req, res);
      });
    }
  });
...
Server.prototype._redirectHost = function(protocol, req, res) {
  var conf = this.conf
    , host = (req.headers['Host'] || '').split(':')[0]
    , path = url.parse(req.url).path
    , rparts = conf.host.split('.')
    , hparts = host.split('.');

  if (conf.host !== hparts.slice(-2).join('.')) {
    host = hparts
      .slice(0, -rparts.length)
      .concat(rparts)
      .join('.');
  }
