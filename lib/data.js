...
Post.get = function(id, func, tag) {
  fs.readFile(Post.getPath(id), 'utf8', function(err, data) {
    ...
    function render() {
      ...
      post.adjacent(tag, function(err, obj) {
...
Post.adjacent = function(post, tag, func) {
  var index = meta.index(post);

  if (!tag) {
    return func(null, {
      previous: meta.get(index - 1), 
      next: meta.get(index + 1)
    });
  }
...
Post.prototype.adjacent = function(tag, func) {
  return Post.adjacent(this, tag, func);
};
