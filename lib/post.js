...
__meta.refresh = function(id) {
  ...
  __meta.keys = Object.keys(docs).sort(function(a, b) {
    a = docs[a.id].timestamp;
    b = docs[b.id].timestamp;
  ...
  if (!stale || stale.updated !== post.updated) {
    if (stale) {
      ...
      tags.forEach(function(tag) {
        if (!~stale.tags.indexOf(tag)) __meta.addTag(tag);
        if (!~post.tags.indexOf(tag)) __meta.removeTag(tag);
      });
    } else {
      post.tags.forEach(__meta.addTag);
    }
    __meta.updateTags();
...
__meta.addTag = function(name) {
  __meta.tags[name] = __meta.tags[name] || 0;
  __meta.tags[name]++;
};

__meta.removeTag = function(name) {
  if (__meta.tags[name]) __meta.tags[name]--;
};

__meta.updateTags = function() {
  Post.tags = Object
    .keys(__meta.tags)
    .sort(function(a, b) {
...
Post.remove = function(post, func) {
  ...
  if (stale) {
    if (stale.tags) {
      stale.tags.forEach(__meta.removeTag);
      __meta.updateTags();
    }
    __meta.remove(id);
  }
    ...
    fs.readdir(config.content, function(err, list) {
      ...
      list.forEach(function(file) {
        ...
        parse(file, function(err, data) {
          ...
          if (data.tags) {
            data.tags.forEach(__meta.addTag);
          }
