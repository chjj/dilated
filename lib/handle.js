...
module.exports = function(server) {
  ...
  list.search = function(req, res, next) {
    ...
    res.local('title', 'Search for ' + utils.escapeHTML(search));

    return Post.search(search, function(err, posts) {
      if (err || !posts.length) {
        return res.error(404,
          'No articles found.');
      }
  ...
  list.tag = function(req, res, next) {
    var tag = req.pathParts[1];
    ...
    res.local('title', 'Posts for ' + utils.escapeHTML(tag));

    return Post.getByTag(tag, function(err, posts) {
      if (err || !posts.length) {
        return res.error(404,
          'No articles found.');
      }
  ...
  list.year = function(req, res, next) {
    ...
    function getYear(callback) {
      ...
      if (!/^\d{4}$/.test(year)) {
        // return res.error(404);
        // NOTE: Should probably do something like:
        // /browse/year/2011 and /browse/tag/foo
        // to avoid collision with years and tags.
        return list.tag(req, res, next);
      }
    ...
    return getYear(function(err, year) {
      if (err) return next(err);
      ...
      if (req.pathParts[1]) {
        res.local('forward', '/browse/' + (year + 1));
      }
      ...
      return Post.range(range, function(err, posts) {
        // XXX Necessary to use this instead of res.error
  ...
  function get(req, res, params, callback) {
    function getPost(callback) {
      return params.slug
        ? Post.get(params.slug, callback, params.tag)
        : Post.getLast(params.tag, callback);
    }

    return getPost(function(err, post) {
      if (err) {
        return req.next();
  ...
  function display(req, res, next) {
    ...
    return get(req, res, params, function(err, post) {
      if (err) return next(err);
      if (res.cached(Post.updated)) return;
  ...
  modify.edit = function(req, res, post) {
    ...
    function renameFile(callback) {
      return id !== old
        ? fs.rename(Post.getPath(old), Post.getPath(id), callback)
        : callback();
    }
    ...
    //return renameFile(function(err) {
  ...
  exports.sitemap = function(req, res, next) {
    ...
    return Post.desc(function(post) {
      posts.push({
        loc: config.host + root + '/' + post.id,
        lastmod: new Date(post.updated).toISOString(),
        priority: '0.5'
      });
    }, function() {
      posts.push({
        loc: config.host + root + '/',
        priority: '0'
      });
  ...
  exports.feed = (function() {
    ...
    return function(req, res, next) {
      ...
      return Post.getLatest(10, function(err, posts) {
