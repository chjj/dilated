// node-pingback - pingbacks for node.js
// (c) Copyright 2011, Christopher Jeffrey (//github.com/chjj)
...
Pingback.prototype._success = (function() {
  ...
  return function(msg, emit) {
    var body = _body.replace('__MESSAGE__', msg);
    this.res.statusCode = 200;
    this.res.setHeader('Content-Length', Buffer.byteLength(body));
    this.res.end(_body, 'utf8');
    this.destroy();
  };
})();
...
Pingback.prototype._handle = function() {
  ...
  var type = this.req.headers['content-type'];
  ...
  var uri = [];
  body.replace(_param, function($0, $1) {
  ...
  if (!target.hostname || (!target.pathname && !target.query)) {
    return this._fault(Pingback.TARGET_CANNOT_BE_USED);
  }

  // make sure there is a potential post id
  if (target.pathname.length < 2 && !target.query.length) {
    return this._fault(Pingback.TARGET_CANNOT_BE_USED);
  }
...
// not necessary, but we lose one level of indentation
Pingback.prototype._pass = function() {
  var self = this, source = this.source, target = this.target;
  ...
  request(source, function(err, res) {
    ...
    self.title = title; // could escape things here
    self.excerpt = excerpt.replace(/\s+/g, ' ').slice(0, 300).trim();
...
Pingback.send = (function() {
  var _body = [
    ...
    '  <params>',
    '    <param><value><string>',
    '      <![CDATA[__SOURCE__]]>',   // maybe remove cdata,
    '    </string></value></param>',  // im sure some terrible CMS's
    '    <param><value><string>',     // probably dont parse it right
    '      <![CDATA[__TARGET__]]>',
    '    </string></value></param>',
    '  </params>',
    '</methodCall>'
  ...
  return function(target, source, func) {
    ...
    request(target, function(err, res) {
      ...
      if (res.headers['x-pingback']) {
        ...
        res.body.replace(/<link[^>]+pingback[^>]+>/i, function(link) {
          link.replace(/href=([^\s>]+)/i, function(__, href) {
            pingback = href.replace(/['"]/g, '').replace(/&amp;/gi, '&').trim();
          });
        });
      }

      if (pingback && !~pingback.indexOf('//')) {
        if (pingback[0] === '/') pingback = pingback.slice(1);
      ...
      var body = _body
        .replace('__SOURCE__', source.href)
        .replace('__TARGET__', target.href);

      request(pingback, body, function(err, res) {
        if (err) return func && func(err);
        if (~res.body.indexOf('fault')) {
          var fault = new Error(), code = res.body.match(
