...
var handler = function(app) {
  ...
  return function(req, res, out) {
    ...
    (function next(err) {
      ...
      if (route) {
        var path = req.pathname, ch = path[route.length];
        if (path.indexOf(route) !== 0 || (ch && ch !== '/')) {
          return next(err);
        }
      }
...
Application.prototype.use = function(route) {
  var self = this, func = slice.call(arguments, 1);
...
Application.prototype._compile = (function() {
  ...
  var load = function(views, temp) {
    var parents = [], i = 0;
    temp = read(join(views, temp), 'utf-8');
  ...
  return function(name) {
    ...
    if (!cache[name]) {
      var template = this.set('engine') || liquor;
...
vanilla.router = function(app) {
  var routes = {}, methods = ['get', 'post', 'put', 'delete'];
  
  methods.concat('all').forEach(function(method) {
    if (method !== 'all') routes[method.toUpperCase()] = [];
    app[method === 'delete' ? 'del' : method] = function(route) { 
  ...
  return function(req, res, out) {
    var stack = routes[req.method === 'HEAD' ? 'GET' : req.method],
        i = 0, pathname = req.pathname, path = req.path[0];
...
vanilla.bodyParser = function(opt) {
  ...
  var multi = opt.multipart && vanilla.multipart(opt);
  var StringDecoder = require('string_decoder').StringDecoder;
  return function(req, res, next) {
    if (req.body || req.method === 'GET' 
      || req.method === 'HEAD') return next();
    var body = '', total = 0, type = req.type;
...
vanilla.auth = function(opt) {
  ...
  var users = opt.users, secret = opt.secret, 
      realm = opt.realm || 'secure area';
  return function(req, res, next) {
    if (req.username) return next();
    var auth = req.headers.authorization;
    if (auth) {
      var s = auth.split(' '), scheme = s[0],
          pair = new Buffer(s[1], 'base64').toString('utf-8').split(':'),
          user = pair[0], pass = pair[1];
